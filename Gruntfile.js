// Generated on 2015-12-13 using generator-ionic 0.7.3
'use strict';
...
module.exports = function(grunt) {
    ...
    grunt.initConfig({
            ...
            development: {
                constants: {
                    ENV: {
                        name: 'development',
                        apiEndPoint: {
                            // mobile: 'http://127.0.0.1:9000',
                            // mobile: 'http://10.10.0.138:9000',
                            mobile: 'http://192.168.43.216:9000',
                            // mobile: 'https://ubertaxi.herokuapp.com',
                            // mobile: 'http://192.168.43.31:3000/',
                            // web: 'http://10.10.0.138:9000'
                            // web: 'http://127.0.0.1:9000'
                            web: 'http://192.168.43.216:9000'
                                // web: 'https://ubertaxi.herokuapp.com'
                        }
                    }
                }
            },
        ...
        watch: {
            ...
            gruntfile: {
                files: ['Gruntfile.js'],
                tasks: ['ngconstant:development', 'jsbeautifier', 'newer:copy:app']
            }
        },
        ...
        copy: {
            ...
            fonts: {
                files: [{
                    ...
                    expand: true,
                    cwd: 'app/bower_components/lato/font/',
                    dest: '<%= yeoman.app %>/fonts/',
                    src: '*'
                }]
            },
    ...
    lo_.functions(cordovaCli).forEach(function(name) {
        grunt.registerTask(name, function() {
            this.args.unshift(name.replace('cordova:', ''));
            // Handle URL's being split up by Grunt because of `:` characters
            if (lo_.contains(this.args, 'http') || lo_.contains(this.args, 'https')) {
                this.args = this.args.slice(0, -2).concat(lo_.last(this.args, 2).join(':'));
            }
            var done = this.async();
    ...
    grunt.registerTask('ripple-emulator', function() {
        grunt.config.set('watch', {
            all: {
                files: lo_.flatten(lo_.pluck(grunt.config.get('watch'), 'files')),
